...
class XenoTicGame {
     ...
     */
    initializeGame() {
        console.log('Initializing XenoTic game...');
    ...
    async handleCellClick(event) {
        console.log('Cell clicked!', event.target.dataset);
        if (this.isProcessing) {
            console.log('Click ignored - game is processing');
            return;
        }
        ...
        if (cell.disabled || cell.textContent !== '') {
            console.log('Click ignored - cell is disabled or occupied');
            return;
        }
    ...
    async startNewGame() {
        try {
            ...
            if (data.success) {
                ...
                // If human chose O (goes second), AI should make the first move
                if (humanSymbol === 'O') {
                    // Small delay to let the UI update, then trigger AI move
                    setTimeout(async () => {
                        await this.triggerAIFirstMove();
                    }, 500);
                }
            } else {
    ...
    /**
     * Trigger AI to make the first move when human chooses O
     */
    async triggerAIFirstMove() {
        try {
            this.isProcessing = true;
            this.showLoadingOverlay(true);
            ...
            if (data.success) {
                ...
                console.error('AI first move failed:', data.error);
            }
        } catch (error) {
     ...
     */
    async getHint() {
        if (this.isProcessing || !this.gameState || this.gameState.game_over) {
            return;
        }
        
        // Show visual feedback that hint is loading
        ...
        try {
            ...
            if (data.success) {
                ...
                console.error('Hint API error:', data.error);
            }
        } catch (error) {
            ...
            this.updateHintButton(); // This will properly set disabled state
        }
    }
